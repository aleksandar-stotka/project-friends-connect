...
function OnlineUsers() {
  ...
  return (
    <>
      ...
      {user && (
        <div className="flex flex-col items-center justify-center bg-blue-100 shadow-lg rounded-lg p-4 my-4">
          <Avatar src={user.photoURL} className="w-16 h-16" />
          <p className="text-gray-700">{user.displayName}</p>
        </div>
      )}
      <div className="max-w-6xl mx-auto p-5 bg-blue-100 shadow-lg rounded-lg">
        {error && <div className="text-red-600">{error}</div>}
        {documents && documents.map((user) => (
          <div key={user.id} className="flex items-center space-x-3 p-2 bg-blue-200 rounded-lg my-2">
